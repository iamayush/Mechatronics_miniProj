...
// Filtered Location
float kalman_loc = 0;
float kalman_loc_old = 0;
float loc_predicted = 0;
int filter_loc = 0;
float step_size = 0;
float K_kf = 0.01;
...
void kalman_filter(void){
    ...
    if (heading == 0){
        if ((filter_loc == 3) || (filter_loc == 4) || (filter_loc == 11) || (filter_loc == 12)) step_size = 0;
        else step_size = 10.0/((float)ONE_CELL_TIME);
    }
    else if (heading == 2){
        if ((filter_loc == 0) || (filter_loc == 7) || (filter_loc == 8) || (filter_loc == 15)) step_size = 0;
        else step_size = -10.0/((float)ONE_CELL_TIME);
    }
    else if (heading == 1){
    ...
    // Discrete Values
    if ((kalman_loc - (int)kalman_loc) < 0.2) filter_loc = (int)kalman_loc;
...
__interrupt void ADC10_ISR(void) {
    ...
    if ((move_type == 'l') || (move_type == 'r') || (move_type == 'e') || (move_type == 'x')){
        obs_loc = obs_loc_old;
    ...
    // mapping
    if (photoR[0] < PHOTOR_THRESHOLD) obs_count++;
    else{
        obs_count = 0;
        obstacle_found = 0;
    }
    if (obs_count >= 10) obstacle_found = 1;
